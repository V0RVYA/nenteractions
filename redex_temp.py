...
class Redex_Push(spa.Network):
    def __init__(self, vocab, theta, rbags, pairs, ports, keys, label = 'push_redex'):
        ...
        hp.add_voc(self.interaction_rules, self.vocab)
        ...
        def get_rule(rports, rtags):
            ...
            def rule(t,x):
                ...
                if state == 0 and tag2_name in self.tags and len(rtags) == 1:
                    state = 1
                elif state == 1:
                    stopwatch = t
                    state = 2
        ...
        def push(rbag_dict, rpair, keys):
            ...
            to_return = 0
            def push_to(t,x):
                nonlocal rbag_dict, rpair, keys, state, sleeptime, stopwatch, to_return 
                rule = x
                rule_name = hp.from_vocab(rule, self.vocab)
                high_rules = ["I_LINK", "I_VOID", "I_ERAS", "I_ANNI"]
                if state == 0 and rule_name in self.interaction_rules and len(rpair) != 0:
                    state = 1
                if state == 1:
                    stopwatch = t
                    state = 2
...
with spa.Network() as model:
    redex_levels = ["HIGH","LOW"]
    hp.add_voc(redex_levels, voc)
    keys = []
    rbag_dict = {"HIGH":{}, "LOW":{}}
    ports_dict = {}
    pairs_dict = {}
    redexes = Redexes(voc, theta, rbag_dict, keys, ports_dict, pairs_dict)
