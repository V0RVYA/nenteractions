...
# Adding port tags
voc.populate("P_MAIN;P_AUX;NULL")
...
table_detector = {
        (voc["P_MAIN"], voc["P_MAIN"]): (voc["Banana"], voc["Apple"], StateVar("statevar1", "bananapass"), 0), 
        (voc["P_AUX"], voc["P_MAIN"]): (voc["Apple"], voc["Banana"], InputVar("a", "dummyin"), 1),
        (None, None): (InputVar(""))
        }
...
with spa.Network() as model:
    ...
    # """
    #
    detect_dfa = DFA(statevars_detector, inputs, outputs, table, voc, start=(voc["Apple"], voc["Banana"], None, 1)) 
